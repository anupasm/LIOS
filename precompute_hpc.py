...
Artifacts:
  ...
  tf_pair_v2_d{bias}_w{weights}_24h_s{seed}_r{rate}_step{step}_range{range}.json

Traffic generation does not calculate routes. Each global Poisson arrival is
allocated to a cross-operator ISL pair using configured source-operator weights.
"""
...
EPOCH = datetime(2026, 6, 14, 0, 0, 0, tzinfo=timezone.utc)
DURATION_SEC = 86_400
...
def _tf_path(cache: Path, seed: int, rate: float, step: int, isl_range: float) -> Path:
    weights_json = json.dumps(
        cfg.simulation.operator_load_weights, sort_keys=True, separators=(",", ":")
    )
